...
xgboost_model = load_model('xbg_model.pkl')
naive_bayes_model = load_model('nb_model.pkl')
random_forest_model = load_model('rf_model.pkl')
decision_tree_model = load_model('dt_model.pkl')
svm_model = load_model('svm_model.pkl')
knn_model = load_model('knn_model.pkl')
voting_classifier_model = load_model('voting_clf2.pkl')
...
def make_prediction(input_df, input_dict):
    probabilities = {
      ...
      'Random Forest':
      random_forest_model.predict_proba(input_df)[0][1],
      'K-Nearest Neighbors':
      knn_model.predict_proba(input_df)[0][1],
    }
...
def explain_prediction(probability, input_dict, surname):
   ...
   Geography_Germany    |    0.091373
    ...
    - If the customer has over 40% risk of churning, generate a 3 sentence explanaition of why they are at risk of churning.
    ...
    Also, don't talk in the first person.
    """

    print("EXPLANAITION PROMPT", prompt)
...
def generate_email(probability, input_dict, explanation, surname):
    prompt = f""" You are a manager at HS Bank, where you are responsible for ensuring a customer stays with the bank and are incentivised with various offers.
    ...
    Generate an email to the customer based on their infomration, asking them to stay if they are at risk of churning, or offering them incentives so that they become more loyal to the bank

    Nake sure to list out a set of incentives to stay based on their infomration, in bullet point format. Don't ever mention the probability of churning, or the machine learning model to the customer.
